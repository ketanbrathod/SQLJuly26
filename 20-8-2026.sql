...
-- DML trigger (Insert, update, delete, Instead of trigger)
-- DDL trigger (Create, alter,drop) Database
-- LogON trigger (Server)

-- Auditing,loging,Compliance, security
...
Alter trigger tr_PreventNegativeSalary
ON Employee
After Update, Insert
AS
Begin
	...
	Begin
		RAISERROR('Please ensure the Salary is non zero',16,1)
		Rollback transaction;
	End
END
...
Create trigger tr_PreventTableCreation
ON Database
for Alter_table
As
Begin
	Print'You are not allowed to drop an Existing Table'
	Rollback transaction;
End
...
Ename varchar(255))
select name from sys.triggers
drop trigger tr_PreventTableCreation on database
...
Create trigger tr_SalaryChangeUpdate
ON Employee
After Update
As
Begin
		Insert into salaryaudit(EID,Oldsalary, NewSalary)
		Select
		i.EID,
		d.Esalary AS OldSalary,
		i.Esalary As NewSalary
		from Inserted i
...
(EID INT,
Ename varchar(255),
deletedON date default Getdate())

Create trigger tr_Deleted_Employee_Audit
ON Employee
After Delete
As
Begin
		Insert into deletedEmployee(EID,Ename)
		Select
		EID,
		Ename
		from deleted 
